...
router.get('/events', async (req, res) => {
  try {
    ...
    const [rows] = await db.execute<RowDataPacket[]>("SELECT * FROM events WHERE event_date >= NOW() ORDER BY event_date ASC");
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: 'Server error' });
  }
...
router.post('/news', authenticate, authorize(['admin', 'alumni_office']), async (req, res) => {
  try {
    ...
    const author_id = (req as any).user.uid; // Get UID from the authenticated user
    ...
    const [result] = await db.execute<ResultSetHeader>(
      'INSERT INTO news (title, content, author_id, target_audience) VALUES (?, ?, ?, ?)',
      [title, content, author_id, target_audience || 'all']
    );
